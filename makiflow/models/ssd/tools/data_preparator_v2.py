...
class DataPreparatorV2:
    ...
    def _collect_image_info(self):
        self._gboxes_xy, self._true_labels = parse_dicts(self._annotation_dict, self._name2num)
    ...
    def resize_images_and_bboxes(self, new_size):
        ...
        """
        images, bboxes = resize_images_and_bboxes_v2(self._images, self._gboxes_xy, new_size)
        self._images = images
        self._gboxes_xy = bboxes
    ...
    def generate_masks_labels_locs(self, default_boxes, iou_threshold=0.5):
        ...
        loc_masks_arr = []
        locs_arr = []
        num_true_boxes = len(self._gboxes_xy)
        ...
        self._last_locs = np.stack(locs_arr)
        return self._last_masks, self._last_labels, self._last_locs

    def generate_masks_labels_offsets_rcnn(self, dboxes_xy, dboxes_wh, iou_threshold=0.5):
        """
        Generates masks, labels and locs for later usage in fit function of the SSD class.

        Parameters
        ----------
        dboxes_xy : array like
            List of default boxes generated by an SSDModel instance. You can get them by
            accessing `default_boxes` field in the SSDModel instance.
        iou_threshold : float
            Jaccard Index default box have to exceed to be marked as positive. Used for
        ...
        num_true_boxes = len(self._gboxes_xy)
        for i in tqdm(range(num_true_boxes)):
            mask, labels, locs = prepare_data_rcnn(
                self._gboxes_xy[i],
                self._true_labels[i],
                dboxes_wh,
                dboxes_xy,
    ...
    def get_bboxes(self):
        return self._gboxes_xy

    def get_true_boxes_labels(self):
        return self._gboxes_xy, self._true_labels
