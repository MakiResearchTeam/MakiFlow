...
class FocalTrainingModule(SegmentatorBasic):
    GAMMA = 'gamma'
    ...
    def _setup_focal_loss_inputs(self):
        self._focal_gamma = tf.placeholder(tf.float32, shape=[], name=FocalTrainingModule.GAMMA)
        if self._use_generator:
            self._focal_num_positives = self._generator.get_iterator()[SegmentIterator.NUM_POSITIVES]
        else:
            self._focal_num_positives = tf.placeholder(tf.float32,
                                                       shape=[self.batch_sz],
                                                       name=FocalTrainingModule.NUM_POSITIVES
            )
    ...
    def fit_focal(self, images, labels, gamma, num_positives, optimizer, epochs=1, global_step=None):
        ...
        try:
            for i in range(epochs):
                ...
                print_train_info(i, (FOCAL_LOSS, focal_loss))
            ...
            if iterator is not None:
                iterator.close()
            return {FOCAL_LOSS: train_focal_losses}
