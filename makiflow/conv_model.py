...
class ConvModel(object):
	...
	def certain_forward(self,X,layer_id=None):
		...
		def through_layers(self,X,layer_id):
			for layer in self.layers[:layer_id+1]:
				X = layer.forward(X,is_training=False)
			return X

		X_calc = through_layers(self,self.X,layer_id)

		X_answer = None

		for i in range(X.shape[0] // self.batch_sz):
			if X_answer is None:
				X_answer = self.session.run( X_calc,feed_dict={self.X : X[i*self.batch_sz:(i+1)*self.batch_sz]} )
			else:
				X_answer = np.vstack( ( X_answer,self.session.run(X_calc,feed_dict={self.X : X[i*self.batch_sz:(i+1)*self.batch_sz]}) ) )
