...
class ODEvaluator:
    def __init__(self, num_processes=1):
        ...
        self.executor = Pool(num_processes)
    ...
    def mean_average_precision(self, detected_bboxes, gt_bboxes, num_classes, iou_trashhold=0.5, verbose=False):
        ...
        for i in range(num_classes):
            detected_boxes_c = Array(DBHolder, [(bytes(bbox[0], 'utf-8'),
                                          bbox[1],
                                          bbox[2],
                                          *bbox[3])
                                          for bbox in detected_bboxes if bbox[1] == i])
            gt_bboxes_c = [[bbox[0], bbox[2]] for bbox in gt_bboxes if bbox[1] == i]
            aps.append( self.executor.apply_async(ODEvaluator.average_precision,
                                                  # Arguments
                                                  args=(
                                                    detected_boxes_c, 
                                                    gt_bboxes_c,
    ...
    def mean_average_precision_multiprocess(self, detected_bboxes, gt_bboxes, num_classes, iou_trashhold=0.5, verbose=False):
        ...
        self.detected_bboxes = Array(DBHolder, [(bytes(bbox[0], 'utf-8'),
                                          bbox[1],
                                          bbox[2],
                                          *bbox[3])
                                          for bbox in detected_bboxes])
        self.gt_bboxes = Array(GTBHolder, [(bytes(bbox[0], 'utf-8'),
                                          bbox[1],
                                          *bbox[2])
        ...
        aps = []
        for i in range(num_classes):
            aps.append( self.executor.apply_async(ODEvaluator.average_precision_multiprocess_exp,
                                                  # Arguments
                                                  args=(
