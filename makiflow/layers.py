...
class ConvLayer(Layer):
    def __init__(self, kw, kh, in_f, out_f, name, stride=1, padding='SAME', activation=tf.nn.relu,
        ...
        kw - kernel width.
        kh - kernel height.
        in_f - number of input feature maps. Treat as color channels if this layer
            is first one.
        out_f - number of output feature maps (number of filters).
        padding - padding mode for convolution operation.
        activation - activation function. Set None if you don't need activation.
        W - filter's weights. This value is used for the filter initialization with pretrained filters.
        b - bias' weights. This value is used for the bias initialization with pretrained bias.
        """
        Layer.__init__(self)
...
class DenseLayer(Layer):
    def __init__(self, input_shape, output_shape, name, activation=tf.nn.relu,
                W=None, b=None):
        """
        input_shape - number represents input shape. Example: 500.
        output_shape - number represents output shape. You can treat it as number of neurons. Example: 100.
        activation - activation function. Set None if you don't need activation.
        W - matrix weights. Used for initialisation dense weights with pretrained weights. 
        b - bias weights. Used for initialisation dense bias with pretrained bias.
        """
        Layer.__init__(self)
...
class BatchNormLayer(Layer):
    def __init__(self, D, name, 
        ...
        mean - batch mean value. Used for initialization mean with pretrained value.
        var - batch variance value. Used for initialization variance with pretrained value.
        gamma - batchnorm gamma value. Used for initialization gamma with pretrained value.
        beta - batchnorm beta value. Used for initialization beta with pretrained value.
