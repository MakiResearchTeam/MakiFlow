...
from makiflow.tools.cv2_tools import apply_op_blur_and_merge
...
class BinaryMaskReaderV2:
    ...
    def load_mask(self, folder_path, mask_shape=None):
        ...
        if self.class_priority:
            label_tensor = self.aggregate_merge(label_tensor, present_classes, marked_zone)

        self.mask_cache[folder_path] = label_tensor
        return label_tensor

    def aggregate_merge(self, masks, present_classes, marked_zone) -> np.ndarray:
