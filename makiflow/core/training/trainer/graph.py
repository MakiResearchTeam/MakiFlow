...
class Graph(TensorProvider):
    ...
    def __init__(self, model):
        ...
        self._layer_name2output_tensors = {}
    ...
    def create_tensor(self, maki_tensor: MakiTensor):
        ...
        if self._layer_name2output_tensors.get(layer.name) is None:
            self._layer_name2output_tensors[layer.name] = dict()

        outputs = self._layer_name2output_tensors.get(layer.name)
        ...
        for tensor in maki_tensor.parent_tensors:
            parent_tensors += [self.create_tensor(tensor)]
